...
<body class="bg-[#EFF6FF] flex h-screen w-full overflow-hidden font-sans">
                    ...
                    <?php foreach($pestañas as $pest): ?>
                        <a href="<?= base_url('pos/filtrar/'.$mesa['id_mesa'].'/'.$categoria['id_categoria'].'/'.urlencode($pest['subcategoria'])) ?>" 
                           class="bg-white p-8 rounded-3xl border-2 border-transparent shadow-sm hover:border-[#1565C0] hover:shadow-xl transition-all flex flex-col items-center justify-center text-center group">
                            <div class="w-16 h-16 bg-blue-50 text-[#1565C0] rounded-2xl flex items-center justify-center text-3xl mb-4 group-hover:bg-[#1565C0] group-hover:text-white transition-colors">⚓</div>
            ...
            <?php else: ?>
                ...
                <div class="grid grid-cols-3 gap-4">
                    <?php foreach($platillos as $p): 
                        // Lógica Sencilla: Si está bloqueado, se bloquea el botón y no abre modal.
                        $onclickAction = "abrirModalSimple(".$p['id_platillo'].", '".addslashes($p['nombre_platillo'])."', ".$p['precio_venta'].")";
                        ...
                        if (!empty($p['nombres_bloqueados'])) {
                            $lista_insumos = implode(", ", $p['nombres_bloqueados']);
                            $onclickAction = "alert('¡Agotado! Falta: " . addslashes($lista_insumos) . "')";
                            ...
                            $badgeOverlay = "<div class='absolute top-2 left-2 right-2 mx-2 bg-red-600 text-white text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider shadow-md z-10 truncate'>🚫 FALTAN: $lista_insumos</div>";
                        } elseif (!empty($p['nombres_alerta'])) {
                            $lista_insumos = implode(", ", $p['nombres_alerta']);
                            $badgeOverlay = "<div class='absolute top-2 left-2 right-2 mx-2 bg-amber-500 text-white text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider shadow-md z-10 animate-pulse truncate'>⚠️ POCO: $lista_insumos</div>";
                        }
                    ?>
    ...
    <dialog id="modalPlatilloSimple" class="bg-white p-6 rounded-3xl shadow-2xl w-full max-w-md backdrop:bg-black/60 backdrop:backdrop-blur-sm">
        <h3 id="modalNombre" class="font-black text-xl text-[#0A1F3D] mb-4 border-b border-gray-100 pb-3">Platillo</h3>
        ...
        <div class="flex gap-4 mb-8 mt-4">
            ...
            <div class="w-3/4">
                <label class="font-black text-gray-700 text-xs uppercase mb-1 block">Comentario / Modificación</label>
                <input type="text" id="modalNota" placeholder="Ej. Sin cebolla..." class="border-2 border-orange-200 bg-orange-50 w-full p-3 rounded-xl font-medium text-orange-900 placeholder-orange-300 focus:outline-none focus:border-orange-400">
            </div>
        </div>

        <div class="flex gap-3">
            <button onclick="document.getElementById('modalPlatilloSimple').close()" class="w-1/3 bg-gray-200 hover:bg-gray-300 text-gray-700 py-4 rounded-xl font-black transition">Cancelar</button>
            <button onclick="guardarAlCarritoSimple()" class="w-2/3 bg-[#1565C0] hover:bg-blue-700 text-white py-4 rounded-xl font-black shadow-lg shadow-blue-500/30 transition flex justify-center items-center gap-2">
