...
<body class="bg-[#F1F5F9] flex flex-col h-screen font-sans overflow-hidden text-[#1E293B]">
    ...
    <header class="bg-[#0A1F3D] text-white flex justify-between items-center px-6 py-4 shrink-0 z-20">
        ...
        <div class="flex gap-4">
            <button type="button" onclick="abrirModalCorte()" class="bg-[#00B4D8] hover:bg-[#0096B4] text-white px-5 py-2 rounded-lg text-sm font-bold flex items-center gap-2">
                <i class="fa-solid fa-wallet"></i> Corte de Caja
            </button>
            <a href="<?= base_url('login/logout') ?>" class="bg-gray-800 hover:bg-gray-700 text-gray-300 px-5 py-2 rounded-lg text-sm font-bold flex items-center gap-2 border border-gray-700">
    ...
    <?php if(session()->getFlashdata('error')): ?>
        <div class="bg-red-600 text-white font-bold text-center py-2 text-sm shrink-0"><?= session()->getFlashdata('error') ?></div>
    <?php endif; ?>
    <?php if(session()->getFlashdata('success')): ?>
        <div class="bg-emerald-500 text-white font-bold text-center py-2 text-sm shrink-0"><?= session()->getFlashdata('success') ?></div>
    <?php endif; ?>

    <main class="flex-1 flex overflow-hidden">
        
        <aside class="w-[320px] bg-white border-r border-gray-200 flex flex-col h-full shrink-0 z-10">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-3 bg-[#F8FAFC]">
                ...
                <?php foreach($mesas as $m): 
                    ...
                    $borderClase = $esActiva ? 'border-[#00B4D8]' : 'border-gray-200 hover:border-gray-300';
                ?>
                    <a href="<?= base_url('caja/index/'.$m['id_mesa']) ?>" class="block bg-white border-2 <?= $borderClase ?> rounded-xl p-4">
        ...
        <section class="flex-1 bg-[#F8FAFC] p-6 overflow-hidden flex flex-col">
            <?php if($mesa_activa): ?>
                ...
                <div class="bg-white rounded-2xl p-6 border border-gray-100 flex justify-between items-center mb-6 shrink-0">
                    <div>
                ...
                <div class="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
                    ...
                    <div class="lg:col-span-7 flex flex-col gap-6 overflow-y-auto pb-2 pr-2">
                        ...
                        <div class="bg-white border border-gray-100 rounded-2xl p-6">
                            ...
                            <div class="flex gap-2 mb-4">
                                <button type="button" onclick="setPropina(0)" id="btn_prop_0" class="btn-prop flex-1 py-3 bg-[#00B4D8] text-white font-bold rounded-xl text-sm border-2 border-[#00B4D8]">0%</button>
                                <button type="button" onclick="setPropina(10)" id="btn_prop_10" class="btn-prop flex-1 py-3 bg-white text-gray-600 font-bold rounded-xl text-sm border-2 border-gray-200 hover:border-gray-300">10%</button>
                                <button type="button" onclick="setPropina(15)" id="btn_prop_15" class="btn-prop flex-1 py-3 bg-white text-gray-600 font-bold rounded-xl text-sm border-2 border-gray-200 hover:border-gray-300">15%</button>
                                <button type="button" onclick="setPropina(20)" id="btn_prop_20" class="btn-prop flex-1 py-3 bg-white text-gray-600 font-bold rounded-xl text-sm border-2 border-gray-200 hover:border-gray-300">20%</button>
                                <input type="number" step="any" id="input_propina_custom" placeholder="Otro %" class="flex-[1.2] py-3 px-2 text-center bg-gray-50 border-2 border-gray-200 font-bold rounded-xl outline-none text-sm focus:border-[#00B4D8] focus:bg-white" oninput="setPropinaCustom(this.value)">
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 h-28 shrink-0 mt-auto">
                            <label class="cursor-pointer h-full">
                                ...
                                <div class="h-full border-2 border-gray-200 bg-white peer-checked:border-emerald-500 peer-checked:text-emerald-600 peer-checked:bg-emerald-50 rounded-2xl flex flex-col items-center justify-center text-gray-400">
                            ...
                            <label class="cursor-pointer h-full">
                                ...
                                <div class="h-full border-2 border-gray-200 bg-white peer-checked:border-blue-500 peer-checked:text-blue-600 peer-checked:bg-blue-50 rounded-2xl flex flex-col items-center justify-center text-gray-400">
                    ...
                    <div class="lg:col-span-5 flex flex-col gap-4 h-full">
                        <div class="border border-gray-200 rounded-2xl p-5 bg-white flex-1 flex flex-col min-h-0">
                            ...
                            <div class="grid grid-cols-3 gap-3 flex-1">
                                <?php foreach([1,2,3,4,5,6,7,8,9,'.',0] as $num): ?>
                                    <button type="button" onclick="tecla('<?= $num ?>')" class="h-full bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:border-gray-300 rounded-xl font-black text-2xl text-gray-700"><?= $num ?></button>
                                <?php endforeach; ?>
                                <button type="button" onclick="tecla('back')" class="h-full bg-red-50 border border-red-100 hover:bg-red-100 text-red-500 rounded-xl font-bold text-xl"><i class="fa-solid fa-delete-left"></i></button>
                            </div>
                        ...
                        <form id="form_cobro_seguro" action="<?= base_url('caja/liquidar') ?>" method="POST" class="shrink-0">
                            ...
                            <button type="submit" id="btn_liquidar_seguro" class="w-full bg-[#00B4D8] hover:bg-[#0096B4] text-white font-black text-lg py-5 rounded-2xl uppercase tracking-widest flex justify-center items-center gap-3">
    ...
    <!-- modal global para el corte de caja plano -->
    <dialog id="modalCorteCaja" class="bg-white rounded-2xl w-full max-w-sm p-0 backdrop:bg-[#0A1F3D]/80">
        <div class="bg-[#0A1F3D] text-white p-5 flex justify-between items-center rounded-t-2xl">
            ...
            <button onclick="document.getElementById('modalCorteCaja').close()" class="text-gray-400 hover:text-white"><i class="fa-solid fa-xmark"></i></button>
        </div>
        ...
        <div class="p-6">
            <div class="flex gap-4 mb-6">
                ...
                <div class="flex-1 border border-gray-200 rounded-xl p-4 bg-white flex flex-col justify-between gap-3">
                    <div>
                    ...
                    <div class="border-t border-gray-100 pt-2">
                        <h4 class="text-[10px] font-black text-[#8BA3BA] tracking-widest uppercase mb-1">Descuentos</h4>
            ...
            <div class="bg-[#F8FAFC] border border-gray-200 rounded-xl p-5 mb-6">
                ...
                <div class="flex justify-between items-center mb-3">
                    ...
                    <input type="number" id="fondo_inicial" value="0.00" class="w-32 border border-gray-300 rounded-lg py-1 px-3 text-right text-sm text-gray-600 font-medium focus:outline-none focus:border-[#0A1F3D] focus:ring-1 focus:ring-[#0A1F3D]" oninput="calcularAuditoria()">
                </div>
                
                <div class="flex justify-between items-center mb-4">
                    <label class="text-sm font-bold text-[#334155]">Efectivo Físico ($)</label>
                    <input type="number" id="efectivo_fisico" value="0.00" class="w-32 border border-gray-300 rounded-lg py-1 px-3 text-right text-sm text-gray-600 font-medium focus:outline-none focus:border-[#0A1F3D] focus:ring-1 focus:ring-[#0A1F3D]" oninput="calcularAuditoria()">
                </div>
            ...
            <form action="<?= base_url('caja/corte_caja') ?>" method="POST">
                ...
                <button type="submit" class="w-full bg-[#0A1F3D] hover:bg-[#1565C0] text-white font-bold py-4 rounded-xl uppercase tracking-widest flex justify-center items-center gap-3 text-sm">
